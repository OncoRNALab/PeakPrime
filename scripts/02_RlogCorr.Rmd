...
```{r}
#loading raw counts, and targeted counts
raw_counts <- read_delim("../data/IMR32_RP_counts.txt")
...
```{r}
plot_data_full<-rbind(plot_data, plot_data_C4)
#write_csv(plot_data_full, "../results/rlog_IMR32.csv")
# Compute correlations between Rp and targeted
...
pearson_cor_imr <- pearson_test$estimate
pearson_pval_imr <- pearson_test$p.value

spearman_cor_imr <- spearman_test$estimate
spearman_pval_imr <- spearman_test$p.value
...
corr_label <- sprintf(
  "Pearson:  %.2f  p = %.2e\nSpearman: %.2f  p = %.2e",
  pearson_cor_imr, pearson_pval_imr,
  spearman_cor_imr, spearman_pval_imr
)
...
```
```{r}
#ggsave("../results/figures/rlog_corr_UHRR_IMR32.png", combined_plot, dpi=300, width = 8, height = 4)
```
