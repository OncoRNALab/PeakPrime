...
UHRR_counts <- read_delim("../data/UHRR_RP_counts.txt")
...
#write.csv(plot_data_C3, "../results/LOG2FC_C3.csv")
```
...
#write.csv(plot_data_C4, "../results/LOG2FC_C4.csv")
```
...
pearson_test <- cor.test(plot_data_full$Targeted,
                         plot_data_full$Random,
                         method = "pearson")

spearman_test <- cor.test(plot_data_full$Targeted,
                         plot_data_full$Random,
                         method = "spearman")
...
# corr_label <- sprintf("Pearson:  %.2f pval: %.2f\nSpearman: %.2f pval: %.2f",
#                       pearson_cor, pearson_pval,spearman_cor, spearman_pval)
...
#ggsave("../results/figures/logFC_UHRR_IMR32_label.png", pltv2, dpi=300, width = 8, height = 6)
#ggsave("../results/figures/Fig_4_pval.pdf", pltv2, dpi=500, width = 8, height = 6)
```
...
geom_point(aes(colour = pool),alpha = 0.5, size = 1) +
  ...
  scale_color_manual(values = color_panel) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", 
                color = "gray40", linewidth = 0.7)+
  labs(
      x = "log2 fold change (targeted) targeted",
      y = "random log2fc"
    ) +
  annotate("text",
           x = lims[1], y = lims[2],
...
p2.bar
#ggsave("../data/results/figures/Bar_logFC_UHRR_IMR32_C4.png", p2.bar, dpi=300, width = 8, height = 6)
```
