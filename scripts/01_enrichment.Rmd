...
FC_counts<- read_delim("./Data/targeted_counts.txt")
#loading and making annotation
ann <- read_csv("Data/TLP11/annotation_TLP11.csv")
sample_names_2<-str_extract(ann$Sample, "RNA\\d+")
ann$Sample <- sample_names_2
colnames(ann)[1]<-"SampleID"

# UHRR and IMR3 Class3 

sample_names <- ann$SampleID[str_detect(ann$Condition, "UHRR_C3_1nM|IMR32_C3_1nM")]
Class3_counts <- FC_counts %>% column_to_rownames(var="ensembl_gene_id") %>% dplyr::select(sample_names) 
sample_names <- ann$SampleID[str_detect(ann$Condition, "UHRR_C4_1nM|IMR32_C4_1nM")]
Class4_counts <- FC_counts %>% column_to_rownames(var="ensembl_gene_id") %>% dplyr::select(sample_names)
...
#ggsave("Data/results/Target_enrichment.png", plt_enrich, dpi = 300, width = 6, height = 4)
```
