...
| `transcriptome_index` | Bowtie1 index prefix | Optional | All index files (`*.ebwt`) must exist | Primer specificity checking via transcriptome alignment | Prefix only (e.g., `transcriptome_idx`) |
...
**Algorithm**: Bowtie1 (v1.3.x) aligns primer sequences to a transcriptome index with parameters optimized for primer specificity checking:
- `bowtie -f -x index primers.fa -S output.sam -a -l 15 -n 3 -v 4 --best`
- Reports all alignments (`-a`) to identify multi-mapping primers
- `-n 3` allows up to 3 mismatches in seed region (key advantage over Bowtie2 for primer checking)
- `-v 4` allows up to 4 mismatches total across entire primer
...
| Bowtie | 1.3.x | Transcriptome alignment | Artistic License |
...
- Transcriptome indices: User-provided Bowtie1 indices matching GTF annotation
...
**Parallelization**:
- MACS2, Bowtie1, and plotting modules are single-process parallel (multi-threaded within the tool)
...
| Transcriptome index | Missing files | Bowtie1 fails with "index not found"; Nextflow halts |
